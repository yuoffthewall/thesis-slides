  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.55\textwidth}
      ...
      \begin{itemize}\setlength{\itemsep}{2pt}
        \item \textbf{Latency:} tens of milliseconds per request.
...
\begin{frame}{The WasmEdge cold-start bottleneck}
  ...
  \vspace{0.3em}
  \alert{A SpiderMonkey workload compiles for 90 seconds; the actual work takes 0.07 seconds.}
\end{frame}
...
\begin{frame}{Execution time is highly skewed}
  ...
  \end{itemize}
  \vspace{0.7em}
  \textbf{The established remedy is tiered compilation.}
  \begin{itemize}\setlength{\itemsep}{2pt}
...
\begin{frame}{Three sub-problems}
  \textbf{Goal.}
  Cut startup latency without giving up the steady-state throughput that LLVM delivers on hot code.
  \vspace{0.4em}
...
\begin{frame}{Contributions}
  ...
  \begin{enumerate}\setlength{\itemsep}{0.5em}
    \item \textbf{\tone{} baseline JIT for WasmEdge.}\\
      Integrates a lightweight Sea-of-Nodes library.
      Uses one uniform calling convention for every function.
      Embeds per-function and per-loop counters directly in compiled code.
    \item \textbf{\ttwo{} selective LLVM-frontend recompilation.}\\
      Synthesizes a mini-module per batch.
      Adds three ABI bridge thunks: \code{fwd\_thunk}, \code{t1\_thunk}, \code{entry\_thunk}.
      Composes batches by walking up the call graph and admitting hot direct callees.
    \item \textbf{On-Stack Replacement.}\\
      Instruments every outermost loop with a three-state back-edge diamond.
      Synthesizes a continuation that starts execution at the loop header.
    \item \textbf{Empirical evaluation.}\\
      Reports a step-by-step ablation on 39 strengthened sightglass kernels.
  \end{enumerate}
\end{frame}
...
\begin{frame}{Pipeline at a glance}
  \begin{itemize}\setlength{\itemsep}{4pt}
    ...
    \item A worker thread serves \ttwo{} batches and OSR continuations off the critical path.
    \item Two notification paths feed two queues: function-entry promotion and loop OSR.
    \item The worker drains the OSR queue first. An OSR continuation must publish before the running loop exits.
...
\begin{frame}{Tier-1: a lightweight Sea-of-Nodes baseline}
  ...
  \end{itemize}
  \vspace{0.5em}
  \textbf{Three design choices distinguish the proposed tier.}
  \begin{enumerate}\setlength{\itemsep}{2pt}
    \item Adopt the Sea-of-Nodes library as the code generator.
    \item Translate wasm to IR in a single pass. The translator emits a PHI only for locals the loop actually modifies. Naive PHI-for-every-local inflates IR by an order of magnitude.
    \item Embed profile counters directly inside compiled bodies. A saturated counter short-circuits in three IR operations.
  \end{enumerate}
\end{frame}
...
\begin{frame}{Calling convention and \texttt{JitExecEnv}}
  ...
  \end{center}
  \vspace{0.2em}
  \scriptsize The worker thread writes only the \code{OsrEntryTable} pointer. The user thread owns every other field.
\end{frame}
...
\begin{frame}{The core idea: mini-module synthesis}
  \textbf{Reuse the LLVM frontend instead of rewriting it.}
  The frontend is mature and already lowers every wasm construct WasmEdge needs.

  \vspace{0.3em}
  \begin{block}{Mini-module synthesis (Algorithm 1)}
    \begin{enumerate}\setlength{\itemsep}{1pt}
      \item Deep-copy the original wasm module.
      \item Replace every defined function outside the batch with a small stub. The stub returns the function's declared type.
      \item Run the LLVM frontend on the resulting mini-module.
    \end{enumerate}
  \end{block}

  \vspace{0.3em}
  \textbf{Why default returns and not traps?}
  A trap stub looks non-returning to LLVM.
  LLVM folds every call site into a trap and deletes the dispatch.
  The post-processing pass then has no call site left to rewrite.
  A default-return stub validates trivially and keeps every call site alive.
\end{frame}
...
\begin{frame}{Three ABI bridge thunks}
  ...
  \end{tabular}
  \vspace{0.3em}
  \scriptsize Each thunk handles one direction. Neither tier needs to know the other tier's ABI.
\end{frame}

\begin{frame}{The compile-ordering invariant}
  \small
  \textbf{Post-processing must run \emph{before} the LLVM optimizer.}
  Reversing the order produces wrong code.
  \vspace{0.4em}

  \textbf{Why the order matters.}
  Before post-processing installs the bridges, every non-batch stub looks pure to LLVM.
  An aggressive pass inlines the stub's return constant into every call site and deletes the dispatch.
  Post-processing then finds no call site to rewrite.

  \vspace{0.4em}
  \textbf{Correct sequence.}
  \begin{enumerate}\setlength{\itemsep}{1pt}
    \item Run the LLVM frontend unoptimized. Every call site survives.
    \item Post-process: install \code{fwd\_thunk}s, rewrite stubs into \code{t1\_thunk}s, prune unused stubs.
    \item Run the LLVM optimizer. The optimizer now sees real call targets.
...
\begin{frame}{Batch composition: walk up, then bounded BFS}
  ...
  A leaf compiled alone still calls back into \tone{} on every helper, so the optimizer cannot inline across the boundary.
  \vspace{0.3em}
  \begin{block}{Algorithm 2 (sketch)}
    \begin{enumerate}\setlength{\itemsep}{1pt}
    ...
    \end{enumerate}
  \end{block}
  \scriptsize The static test admits siblings whose dynamic counters lag behind but that the optimizer will still benefit from seeing.
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.55\textwidth}
      ...
      \begin{itemize}\setlength{\itemsep}{1pt}
        \item The running \tone{} frame finishes the loop and returns.
        \item The entry-table swap fires too late to matter.
        \item No future call ever observes the swap.
      \end{itemize}
    ...
    \begin{column}{0.42\textwidth}
      ...
      \begin{tikzpicture}[font=\scriptsize,node distance=3mm]
        ...
        \node[draw,rounded corners,fill=tier2!20,right=of loop,xshift=-2mm,minimum width=2cm] (swap) {entry swap};
        \node[draw,rounded corners,fill=tier1!15,below=of loop,minimum width=3.5cm] (ret) {return};
        \draw[-{Latex}] (call) -- (loop);
        \draw[-{Latex}] (loop) -- (ret);
        \draw[->,dashed,tier2] (swap.west) -- (loop.east);
        \node[below=of ret,font=\scriptsize\itshape,color=tier2] {swap arrives too late};
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}[fragile]{Three-state back-edge instrumentation}
  Each outermost loop reads one slot of \code{OsrEntryTable} on every back-edge.
  The slot encodes a three-state machine: \textbf{counting} $\to$ \textbf{waiting} $\to$ \textbf{ready}.
...
\begin{frame}{Methodology}
  ...
  \begin{itemize}\setlength{\itemsep}{1pt}
    \item \textbf{Frequent-call} (16): one hot function called many times.
    \item \textbf{Hot-leaf} (6): the threshold-tripper sits below the real hot work.
    ...
    \item \textbf{Compile-dominated} (6): LLVM compile cost dwarfs the wasm work.
  \end{itemize}
  \vspace{0.3em}
...
\begin{frame}{The largest wall-clock wins}
  ...
  \begin{tabular}{lrrl}
    ...
    \code{image-classification}            & --- & --- & 8.8$\times$ \\
    \code{sqlite3}                         & --- & --- & 8.5$\times$ \\
    \bottomrule
  \end{tabular}
  ...
  Each module embeds an interpreter, a reflection-driven dispatcher, or a graph of large tensor operators.
...
\begin{frame}{Summary}
  \footnotesize
  This thesis presents a \textbf{tiered compilation architecture for WasmEdge} that separates startup latency from peak code quality.
...
\begin{frame}{Backup: workload categorization (39 kernels)}
  ...
  \begin{description}\setlength{\itemsep}{3pt}
    \item[Frequent-call (16)] \code{blake3-scalar, bz2, gcc-loops, hashset, pulldown-cmark, quicksort, regex, shootout-gimli, shootout-heapsort, shootout-keccak, shootout-memmove, shootout-seqhash, shootout-sieve, shootout-switch, shootout-xblabla20, shootout-xchacha20, tinygo-regex}
    ...
    \item[Compile-dominated (6)] \code{image-classification, spidermonkey-json, spidermonkey-markdown, spidermonkey-regex, sqlite3, tinygo-json, tract-onnx-image-classification}
  \end{description}
  \vspace{0.3em}
